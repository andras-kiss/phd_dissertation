...
To test this hypothesis, step response measurements were carried out in unstirred and stirred buffers. If there is indeed an adhered layer delaying the response, by stirring the buffers when they are exchanged, the time constant should be the much lower RC time constant, with R = 218.86 $\ohm$. 
...
The buffer solutions were quickly changed back and forth between pH = 4 ($\sim \; -240$ mV) and pH = 7 ($\sim \; -340$ mV).
