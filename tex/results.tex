...
The problem stems from the large time-constant.
For a sufficiently small microelectrode, the resistance \emph{R} can be so high, that the RC time-constant is measured in seconds.
To arrive at the equilibrium potential of the indicator electrode, about 4$\tau$ time must be allowed for the cell.
Such high equilibration period is not practical, since a high number of sampling points must be included in the image raster to achieve the desired high resolution, and the 4$\tau$ equilibration period must be waited at every sampling point.
...
For instance\footnote{In case where the rate determining step is the charging of the capacitor in the RC filter model of the potentiometric cell.}, if \emph{R} = 1 G$\ohm$ and \emph{C} = 500 pF, then \emph{RC} = 0.5 s, and $t$ = 882 s, or about 16 min, since about $t_e = 4 \times RC$ is necessary to obtain the equilibrium electrode potential with good approximation.
...
The solid contact electrode had a slightly wider dynamic range.
...
Step size: 25 $\upmu$m and 50 $\upmu$m on the X and Y axes.}
...
Substituting the value of D = 7.06$\times$10$^{-8}$ dm$^2 \cdot$s$^{-1}$ \cite{lide2001crc}, $C_s=3.29\times10^{-2}$ M (surface concentration at $t~$=10$~$min), a = 0.0038 dm, the result is $\Omega$ = $3.53\times10^{-11}$ mol$\cdot$s$^{-1}$.
...
\end{axis}
\end{tikzpicture}
\caption[Retracting and lateral SECM linescans above the AZ63 sample initiated at different instances in time.]{(A, B) Retracting and (C) lateral SECM linescans above the AZ63 magnesium-aluminium-zinc alloy sample initiated at different instances in time.
...
Corrosion current at $\sim$10 minutes\footnote{The experiment involved interchanging resistors manually. It was carried out in less than 2 minutes, centered around 10 minutes after the galvanic coupling was established.} between the Mg/Al sample and four Fe samples with different diameters was also measured directly.
...
Ion flow rates from Mg/Al samples coupled with Fe samples of different diameters are proportional to the surface area of the sample; $\Omega_{\textrm{0.59~mm}} = 4.60\cdot10^{-11}~$mol/s, $\Omega_{\textrm{1.2~mm}} = 4.66\cdot10^{-11}~$mol/s, $\Omega_{\textrm{2.3~mm}} = 1.26\cdot10^{-10}~$mol/s.
...
Mean squared error is $9.63\cdot 10^{-3}$ and $2.95\cdot 10^{-3}$ for the images scanned with the web and the arc algorithms, respectively\footnote{Mean squared error was calculated by averaging the square of the differences between the input and the output of the SECM simulations.}.
...
\centering
% trim = top left bottom right
\includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/meander_sim.eps} \includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/meander_delta.eps}

\includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/fastcomb_sim.eps} \includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/fastcomb_delta.eps}

\includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/comb_sim.eps} \includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/comb_delta.eps}

\includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/web_sim.eps} \includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/web_delta.eps}

\includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/arc_sim.eps} \includegraphics[trim = 10mm 30mm 0mm 20mm, clip, width=0.25\textwidth, angle=-90]{img/sim/arc_delta.eps}
...
Step size was 100 $\upmu$m on both axes for the Cartesian coordinate based images.
Arc length was 100 $\upmu$m for the ,,arc'' algorithm.
Step size varied for the ,,web'' algorithm.}
...
Indicator electrode was a pH-sensitive antimony micro-electrode.
Slope of the cell employing the antimony micro-electrode was 44 mV / pH unit.
...
Indicator electrode was an antimony pH microelectrode.
Potential was measured against an Ag/AgCl/3M KCl.
Slope of the cell employing the antimony micro-electrode was 45.1 mV / pH unit.
...
\newpage
\section{The effect of electric field on potentiometric SECM imaging}
...
However, an electric field is also formed as a result of the potential difference between the surfaces of the galvanic pair, which has a direct influence on the potential of the indicator microelectrode, as it is depicted in Fig. \ref{fig:field}. The measured potential is the sum of these two contributions:
...
where $\Delta E$ is the measured potential difference, $E_M$ and $E_R$ is the potential of the indicator\footnote{\textbf{M} for measuring electrode, which is \emph{indicator electrode} according to the IUPAC recommendation.} and the reference electrode, and $\phi_M$ and $\phi_R$ are the local potentials in the electric field at the indicator and reference electrodes, respectively.

\begin{figure}[b]
\centering
% trim = top left bottom right
\includegraphics[width=0.6\textwidth]{img/field/field.eps}
\caption[An electric field is formed between the surfaces of the galvanic couple.]{An electric field is formed between the surfaces of the galvanic couple. The potential difference between the indicator ($\phi_M$) and reference ($\phi_R$) electrodes is added to the Nernstian potential associated with the activity of the primary ion.}
\label{fig:field}
\end{figure}
...
The SECM probe was a solid contact magnesium ion selective microelectrode.
...
The first 6 measurements were conducted while the AZ63 sample was electrically isolated from the iron sample (red lines, a-b).
...
Indeed, such a 40 mV change would correspond to an increase of $\sim$ 1.5 orders of magnitude in Mg$^{2+}$ activity occuring in less than one second.
...
During this period of galvanic coupling, the potential sensed at the ISME, when situated at h = 1000 $\upmu$m, increased by $\sim$ 40 mV.
This rise ($\Delta E_2$) can be totally attributed to the increase in activity of the dissolving metal, i.e.: $\Delta E = 29.5~\textrm{mV} \cdot \Delta \lg[$Mg$^{2+}]$.
