...
\centering
\begin{tikzpicture}
\begin{axis}[xmin=-30, xmax=100, width=12cm,height=6cm,xlabel={time, s}, ylabel={E vs. Ag/AgCl/3 M KCl, mV}, clip marker paths=true, width=12cm, height=8cm, legend style={draw=none}, legend cell align=left]
...
\addplot [domain=0:100, samples=500, color=blue, line width=0.4mm] {-280+(280-183)*(exp(-(x)/9) + exp(-(x)/0.5577))/2};
...
\addlegendentry{$y = - 280 + 97 e^{-x/3.76}$}
\addlegendentry{$y = - 280 + 97 (e^{-x/9} + e^{-x/0.5577})/2$}
\end{axis}
\end{tikzpicture}
