...
%Three, more or less independent resear
...
The main merit of the work of the early pioneers from the standpoint of the electrochemist, is the foundation of the basics of the micropipette techniques.
Including the preparation, instrumentation, and basic characterization of them.
...
The best subject for electroneurophysiological studies is the giant squid axon.
...
Interestingly, the Second World War might have induced the further miniaturization of the microelectrodes, because it restricted the availability of the giant squid.
...
In order to carry out voltage clamp experiments, the intracellular microelectrodes had to be miniaturized further.
...
It was in 1906, when Cremer discovered that the potential difference across a thin glass membrane is a function of pH when opposite sides of the membrane are in contact with solutions containing different concentrations of H$_3$O$^+$ \cite{cremer1906ursache, cremer1906direkte}.
...
This however, is not entirely true, because pH depends on the \emph{activity} of H$_3$O$^+$, rather than the concentration:
...
\begin{equation}
pH = -lg(\gamma_{H_3O^+} m_{H_3O^+} / m^\theta)
\end{equation}

or

\begin{equation}
pH = -lg(\gamma_{H_3O^+} c_{H_3O^+} / c^\theta)
\end{equation}

where $m^\theta = 1 mol kg^{-1}$ and $c^\theta = 1 mol dm^{-3}$ are the standard states, and $\gamma_{H_3O^+}$ is the activity coefficient of $H_3O^+$. 
...
\begin{equation}
Pt(s) | H_2(g) | \text{buffer solution}, Cl^-(aq) | AgCl(s) | Ag(s)
\label{eq:harned}
\end{equation}
...
\begin{equation}
E = E^\theta - \frac{RT}{z_iF}lna_i
\end{equation}

where $E^\theta$ is the standard potential difference of the cell, R the universal gas constant, F the Faraday constant, T the thermodynamic temperature, z$_i$ is the valence and a$_i$ is the activity of ion species i.
...
\begin{equation}
E = E^\theta - \frac{RTln10}{F}lg\bigg(\frac{a_{H_3O^+}m_{Cl^-}\gamma_{Cl^-}}{m^\theta}\bigg)
\end{equation}

which can be rearranged to give the pH:

\begin{equation}
p(a_{H_3O^+}\gamma_{Cl^-}) = -lg(a_{H_3O^+}\gamma_{Cl^-}) = \frac{E - E^\theta}{(RT/F)ln10} + lg \bigg(\frac{m_{Cl^-}}{m^\theta}\bigg)
\end{equation}

where $\gamma_{Cl^-}$ is the molal activity coefficient of the chloride ions at the molality $m_{Cl^-}$.

The acidity function value at zero chloride molality $pa_0 = -lg(a_H \gamma _{Cl^-})m_{Cl^- \to 0}$  is determined by linear extrapolation of the acidity function values obtained by fitting by least square method.
Calculation of trace activity coefficient of chloride ions $\gamma_{Cl^- \to 0}$ at $m_{Cl^- \to 0}$ according to the Bates-Guggenheim convention \cite{bates1960report}:

\begin{equation}
lg \gamma_{Cl^- \to 0} = \frac{A I^{1/2}}{1 + 1.5I^{1/2}}
\end{equation}
...
\begin{equation}
Ag | AgCl | \text{inner solution} || \text{membrane} || \text{sample} | 0.1 M NH_4NO_3 | KCl std., Hg_2Cl_2 | Hg
\end{equation}
...
\begin{quote}
\vspace{0.5cm}
\emph{,,Valinomycin-based microelectrodes will find increasing application only if their electrical resistances can be further reduced without inducing a loss in K$^+$ selectivity.''}
\vspace{0.5cm}
\end{quote}
...
The big breakthrough came when Ammann realized, that the resistance of neutral carrier-based membranes can be lowered drastically if both lipophilic salts and polar membrane solvents are used to form the membrane phase \cite{ammann1987valinomycin}.
...
\caption{(A) Valinomycin, (C) nonactin, ionophores for potassium ions, and (B) bis-N,N-dicyclohexyl-malonamide, ionophor for magnesium ions.}
...
An important area of application of ion-selective electrodes is blood analysis.
Most often, doctors are interested in concentrations of sodium, potassium, calcium, and hydrogen ions.
In the sixties and early seventies, most of these biological ion measurements were made by the method of flame photometry.
This method works very well in a central laboratory of a clinic, but not necessarily very adequate for a doctors office.
But in the sixties, the space program was extremely important, and measurements had to be made in space.
It meant that the measurement should be simple, fast, it should be easy to use under difficult circumstances.
Ion-selective cells were developed partially to make those measurements in space.
A voltage measurement is very simple, it is cheap, and doesn't require complicated instruments, low powered, and also, very importantly, easy to miniaturize with micro fabrication techniques.
It means, that very small volumes of samples can be analyzed in a very short period of time.

As it was mentioned it is important in clinical diagnostics to measure certain ions in blood.
For example it is very important to decide very quickly, whether someone with a chest pain has a heartburn or a heart attack.
Surprisingly, it is a very difficult question to answer.
If we keep someone with heartburn in the ER, it's extremely expensive.
If we send home somebody with an acute myocardial infarction, they may die.
There are instruments to measure the important ions with diagnostic value, but again, in central laboratories.
That's why it was -- and still is -- very important to develop ion-selective electrodes to minimize the time between patient admission and treatment.
...
These are not based on ionophores, but will be described here briefly because they are related closely to the just mentioned blood analyzers.
...
Solid-contact electrodes were used in the work done in our laboratory by Gergely Gyetvai \cite{gyetvai2007solid}, Ágnes Varga \cite{varga2011development} and Javier Izquierdo \cite{izquierdo2012scanning}.
...
One of the most often used type of these is the $Ir/IrO_2$ electrode \cite{beyenal2004improved}.
The oldest is certainly the $Sb/Sb_2O_3$ electrode, its initial characterization dating back to 1923 \cite{uhl1923electrometric}.
...
The main reason this particular electrode is so popular is that the melting point of antimony and borosilicate glass are similar ($T_{m, Sb} = 630.63 \celsius, T_{m, glass}\approx 700 \celsius)$, and manufacturing them is relatively easy with standard glass blowing techniques.
...
Unfortunalety, as with any other technique, the measurement itself alters the measured value.
This effect is especially large in potentiometry.
...
It can be split into three parts \cite{halliwell1987using}:


\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{img/theory/loading_error.eps}
\caption[The generation and measurement of the a potential of a microelectrode.]{The generation and measurement of the a potential of a microelectrode.
The circuit can be split into 3 sections: (A) voltage source, the potential difference between the reference and the measuring electrodes with an $R_0$ output resistance.
(B) The measuring circuit, which consists of 3 elements: an ideal voltmeter, a resistance $R_{in}$ to account for current flowing in the input in response to the input voltage, and a capacitance $C_{in}$.
(C) The electrical connection between the voltage generator and the measuring circuit, with a capacitance $C_s$.}
...
Although $E$ is regarded perfect, and it produces a constant voltage even if very large currents are generated, the ouput voltage $V$ is reduced by an amount $E - V = iR_0$.
...
$V$, an ideal voltmeter, which draws no current from the circuit and responds instantly to potential changes at the input.
The second element is a resistance $R_{in}$ to account for the imperfection of real voltmeters, the current flowing in the input in response to the input voltage.
The third element is the input capacitance of the voltmeter, modeled by the capacitor $C_{in}$.
The current drawn by the input is $V/R_{in}$ and determines the error in measuring $E$:

\begin{equation}
\label{eq:loading}
	E - V = \frac{R_0 V}{R_{in}}
\end{equation}
...
The input resistance, $R_{in}$, of a typical voltage meter is often 1 $M\ohm$ or 10 $M\ohm$, of a microelectrode amplifier $10^{12} \ohm$ and of a pH meter $10^{14} \ohm$.
It is clearly important that $R_{in} \gg R_o$ to minimize the error in potential measurements.
The input capacitance contributes to slowing the response to a change of $V$, to an extent that depends on $C_{in}$ and $R_{in}$; the output for a step input is an exponential of time constant $\tau = C_{in}R_{in}R_0/(R_{in}+R_0)$.

\emph{(C) The connection} between voltage generator and measuring instrument usually involves a wire of low resistance, but often with important stray capacitance to ground.
In the case of screened cable (with braided copper shield connected to earth) this amounts to 100-200 pF/m and may restrict the speed of transmitted signals if it is in series with a large output impedence.
A second effect of this is the source of noise in the measured voltage.
...
In the case of a microelectrode or other high resistance source, the stray capacitance should be kept as small as possible by using short connections between the electrode and amplifier input.

Operational amplifiers are used to solve several of the issues described above.
...
In this configuration, the entire output voltage is fed back into the inverting input.
...
$V_{out}$ follows $V_{in}$, so the circuit is named op-amp voltage follower.
...
The input impedance of the op-amp is very high (1 $M\ohm$ to 10 $T\ohm$), meaning that the input of the op-amp does not load down the source and draws only minimal current from it.
Because of this, $E - V = iR_0$ is much less, and the measured $V$ is almost identical to the source $E$.
...
Since $i$ is nothing but the change of charge in time, $i = dq / dt$, and $V_{out} = q / C$, Eq. \ref{eq:loop1} can be rewritten as

\begin{equation}
\label{eq:loop2}
        V_{in} - \frac{dq}{dt}R - \frac{q}{C} = 0
\end{equation}
...
\begin{equation}
\label{eq:loop3}
        \frac{dq}{dt} = \frac{1}{R}\bigg(V_{in} - \frac{q}{C}\bigg)
\end{equation}
...
\begin{equation}
\label{eq:loop4}
        \frac{dq}{V_{in}-\displaystyle{\frac{q}{C}}} = \frac{dt}{R}
\end{equation}
...
\begin{equation}
\label{eq:loop6}
        \int_{0}^{q} \frac{dq}{q - CV_{in}} = \int_{0}^{t} - \frac{dt}{RC}
\end{equation}
