...
In this section I look at the motivation behind electrode miniaturization, and the difficulties the researchers at those times faced.
...
\begin{equation}
\text{Ag | AgCl | KCl(aq) || membrane || sample | NH$_4$NO$_3$ | KCl(aq) | Hg$_2$Cl$_2$| Hg}
\end{equation}
...
Throughout my dissertation I use the expression \emph{equilibrium} to describe a steady potential difference between the measuring and the reference electrodes. However, this state of course cannot be regarded an equilibrium, since given enough time, the potential difference will reach zero. The resistance of the whole cell is finite, and if there is a potential difference between any two points in the cell, current will flow. Two phenomena have to be distinguished. The first is the electrochemical process, which in real cells ($i \neq 0$) is in equilibrium \emph{only} if cell potential difference have reached zero, i.e. the cell is depleted. The other process is the charging or discharging of a capacitance modeled by $C$ in Fig. \ref{fig:rc}. If we accept the model described in the previous section, this is can be regarded a purely physical process, which is in true eqilibrium if the voltage across the capacitor $C$ is the same as the voltage of the input ($V_i = V_o$). For this argument to be true, the source of the potential difference must not change over time given an electrolyte composition. This is true in the timeframe of the measurements in this thesis. Therefore, I use the expression \emph{equilibrium} to refer to a state of the potentiometric cell, in which the voltage across the plates of the capacitor does not change over time, and equal to $V_i$ and $V_o$.
